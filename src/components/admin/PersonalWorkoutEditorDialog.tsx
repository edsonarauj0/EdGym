            ...
            <div className="space-y-3">
              {exercises.map((exercise, index) => (
                <div key={exercise.id} className="rounded-lg border p-3 space-y-3">
                  ...
                  <div className="flex gap-2"><Input value={exercise.videoSearchQuery} onChange={(event) => updateExercise(exercise.id, { videoSearchQuery: event.target.value })} placeholder="Busca do vídeo no YouTube" /><a href={getYouTubeSearchUrl(exercise.videoSearchQuery || exercise.name)} target="_blank" rel="noopener noreferrer" className="inline-flex shrink-0 items-center gap-1 rounded-lg border px-3 text-sm font-medium text-primary hover:bg-primary/5" title="Ver vídeo no YouTube"><ExternalLink className="size-4" /> Vídeo</a></div>
                </div>
              ))}
