...
export function UserProfilePage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <Card className="border-primary/25"><CardContent className="flex items-center gap-4 p-5"><div className="flex size-14 items-center justify-center rounded-lg bg-primary/15 text-xl font-bold text-primary">{user.name?.charAt(0).toUpperCase() || 'U'}</div><div className="min-w-0 flex-1"><h1 className="text-2xl font-bold">{user.name}</h1><p className="text-muted-foreground">{user.email}</p></div><Badge>{user.assignedGroupIds?.length || 0} grupos</Badge></CardContent></Card>
